...
# Existing Pravha imports
from your_existing_modules import *

# NEW: Mesh network imports
from mesh_network.core.mesh_node import MeshNode
from mesh_network.integration.pravha_bridge import PravhaBridge
from mesh_network.models.message_models import FloodAlert, SOSMessage
from utils.mesh_integration import MeshIntegrationManager

# Global mesh network components
mesh_node = None
pravha_bridge = None
...
async def lifespan(app: FastAPI):
    """Manage app lifecycle with mesh network"""
    global mesh_node, pravha_bridge, mesh_integration
    
    try:
        # Start existing Pravha services
        logging.info("Starting Pravha services...")
        
        # NEW: Initialize mesh network
        logging.info("Initializing mesh network...")
        mesh_node = MeshNode(
            device_id="PRAVHA_CENTRAL_001",
            message_callback=handle_mesh_message
        )
        
        pravha_bridge = PravhaBridge(
            mesh_node=mesh_node,
            pravha_api_url="http://localhost:8002",
            bridge_id="PRAVHA_BRIDGE"
        )
        
        mesh_integration = MeshIntegrationManager(
            mesh_node=mesh_node,
            pravha_bridge=pravha_bridge
        )
        
        # Start mesh network
        await asyncio.gather(
            mesh_node.start_node(),
            pravha_bridge.start_bridge(),
        ...
        # Cleanup
        if pravha_bridge:
            await pravha_bridge.stop_bridge()
        if mesh_node:
            await mesh_node.stop_node()
        if mesh_integration:
            await mesh_integration.stop()

# Create FastAPI app with lifespan
app = FastAPI(
    title="Pravha - Flood Forecasting & Mesh Network",
    description="AI-powered flood prediction with mesh network communication",
    version="2.0.0",
    lifespan=lifespan
)
...
# NEW: Mesh message handler
async def handle_mesh_message(message):
    """Handle messages received from mesh network"""
    try:
        if message.type == "SOS":
            await process_emergency_sos(message)
        elif message.type == "FLOOD_ALERT":
            await update_flood_status(message)
        elif message.type == "LOCATION_UPDATE":
            await update_device_location(message)
    except Exception as e:
        logging.error(f"Failed to handle mesh message: {e}")

# Existing Pravha routes...
@app.get("/")
async def root():
    return {"message": "Pravha Flood Forecasting with Mesh Network"}

@app.post("/predict")
async def predict_flood(location_data):
    # Your existing flood prediction logic
    prediction = your_existing_prediction_logic(location_data)
    
    # NEW: If high risk, broadcast through mesh network
    if prediction.get('risk_level', 0) > 0.7:
        await broadcast_flood_prediction(prediction)
    
    return prediction

# NEW: Mesh network API routes
@app.get("/api/mesh/status")
async def get_mesh_status():
    """Get mesh network status"""
    if not mesh_integration:
        raise HTTPException(status_code=503, detail="Mesh network not initialized")
    
    return mesh_integration.get_status()

@app.get("/api/mesh/coverage")
async def get_mesh_coverage():
    """Get mesh network coverage data"""
    if not mesh_integration:
        raise HTTPException(status_code=503, detail="Mesh network not initialized")
    
    return mesh_integration.get_coverage_data()

@app.post("/api/mesh/broadcast-alert")
async def broadcast_emergency_alert(alert_data: dict):
    """Broadcast emergency alert through mesh network"""
    if not mesh_integration:
        raise HTTPException(status_code=503, detail="Mesh network not initialized")
    
    success = await mesh_integration.broadcast_alert(alert_data)
...
@app.get("/api/mesh/emergency-reports")
async def get_emergency_reports():
    """Get emergency reports from mesh network"""
    if not mesh_integration:
        raise HTTPException(status_code=503, detail="Mesh network not initialized")
    
    return mesh_integration.get_emergency_reports()

# NEW: Helper functions
async def broadcast_flood_prediction(prediction):
    """Broadcast flood prediction through mesh network"""
    try:
        flood_alert = FloodAlert(
            source_device_id="PRAVHA_CENTRAL_001",
            content=f"FLOOD ALERT: {prediction.get('message', 'High flood risk detected')}",
            flood_level=prediction.get('predicted_level', 0.0),
            evacuation_zones=prediction.get('affected_areas', []),
            safe_routes=prediction.get('safe_routes', [])
        )
        
        await mesh_node.send_message(flood_alert)
        logging.info("Flood prediction broadcasted through mesh network")
        
    except Exception as e:
        logging.error(f"Failed to broadcast flood prediction: {e}")

async def process_emergency_sos(sos_message):
    """Process SOS message from mesh network"""
    try:
        # Store in database
        emergency_data = {
            "device_id": sos_message.source_device_id,
            "timestamp": sos_message.timestamp,
            "location": sos_message.coordinates,
            "emergency_type": sos_message.emergency_type,
            "casualties": sos_message.casualties,
            "resources_needed": sos_message.resources_needed,
            "status": "ACTIVE"
        }
        
        # Your existing emergency processing logic
        await store_emergency_report(emergency_data)
        
        # Trigger response coordination
        await coordinate_emergency_response(emergency_data)
        
        logging.info(f"Processed SOS from {sos_message.source_device_id}")
        
    except Exception as e:
        logging.error(f"Failed to process SOS: {e}")

# Your existing functions...
# (keep all your current Pravha logic)